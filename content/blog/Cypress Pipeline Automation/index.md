...
```
it would basically create a set of tasks once imported that enables tests to run but before that to extract archived files as well as to swapping of variables from the cyress.deployment.json file that needs to be created for all the environments and all variables will need to follow __variable1__ syntax in order to be swapped with that stage's value.
...
Name -> variable1 (without the __ as these will be swapped with this value now)
Value -> e.g 536 
